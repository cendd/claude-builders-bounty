...
- Never delete migrations — create new ones to reverse changes
...
- Don't create wrapper components around shadcn/ui without a good reason
...
| Anti-pattern | Why |
|---|---|
| Using `fetch` in `useEffect` | Breaks RSC streaming, no caching |
| Storing files in SQLite | Use S3/R2 for file uploads |
| Mixing ORM styles | Pick Drizzle or Prisma, not both |
| Running migrations on every deploy | Use CI migration step, not app startup |
| Over-abstracting early | Wait for 3 occurrences before extracting a shared utility |
| Ignoring SQLite WAL mode | Always enable WAL for concurrent reads |
